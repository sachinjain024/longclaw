 ...
 *
 * **The box is 107px at the width where it is smallest**, and 107 / 6.3 is 16.
 * The narrowest box is the one that decides, because this is one constant for
 * every window: the chip has 115px of room at 1180 and 107 below about 900,
 * where the shell squeezes the panel, and a cap taken from the wide
 * measurement clips at the narrow one — visibly, as a second ellipsis after
 * the first.
 *
 * Every attempt to *derive* this from the panel's width was wrong, four times:
 * 22 characters in a box calculated at 140 and measured at 123; 25 in a box of
 * 157, over by a pixel; 24 in a box that turned out to have the settings gear
 * hanging over the end of it; 18 from the box at 1180, which clips at 900. So
 * it is measured, and `probe:header` measures it again at every width on every
