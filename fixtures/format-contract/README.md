...
| A comment carries `edited_at` beside `occurred_at`, and the stream still orders on `occurred_at` | A comment's author may rewrite their own words, and a fixed typo must not send the comment to the end of the conversation (LC-241q) | `valid-comment-edited-in-place` |
| `edited_at` is held to the timestamp rule `occurred_at` is held to | It is a timestamp, and a record whose stated instant cannot be read is a record that cannot be read | `degraded-record-invalid-edited-at` |
...
| The four opt-in properties are read as text, and a value this configuration cannot interpret is kept | Invariant 16. An estimate written under another system, a type slug nothing defines, a date in a shape the format does not store — parsing them here would be the place that stopped being true, and a missing pair of quotes would cost a ticket | `valid-ticket-properties-uninterpretable` |
| A property key this build knows is never an unknown key | Invariant 11 covers a key the build does not know; 16 covers a key it knows and a value it cannot read. A reader that conflated them would report the wrong thing about both | `valid-ticket-properties` |
