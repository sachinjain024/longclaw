...
2. Check out the annotated tag `tauri-v2-architecture-m2`, which preserves the
   spike with live manifests (`git worktree add /tmp/spike tauri-v2-architecture-m2`),
   then run `cd /tmp/spike/spikes/tauri-v2-architecture && npm install && npm run verify && npm run spike`.
