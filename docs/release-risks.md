...
| ~~Registry corruption strands every known project path~~ **Retired 2026-08-02 (V0-31)** | Persistence | V0-31, [plan 38](plans/completed/38-complete-step-14-recovery.md) | Met. `RegistryStore` preserves `project-registry.backup.json`, fails closed on invalid registry JSON, has restore-from-backup and no-overwrite regression tests, and documents recovery in `apps/desktop/README.md` |
