...
| Risk | Owner | Retired by | Must pass before release |
|---|---|---|---|
| ~~A large board becomes slow enough to feel broken; the spike proved data flow, not 5,000 rendered cards~~ **Retired 2026-07-31 (V0-06)** | Frontend | V0-06, [plan 07](plans/completed/07-board-virtualization.md) | Met. `npm run perf:board` traces a 5,000-ticket board in WebKit: 18 ms p95 keyboard navigation, 21 ms p95 scroll, 18 ms p95 external write → paint, against the ≤ 50 ms p95 budget, and indistinguishable from a 600-ticket board. The list surface (V0-14) inherits the same lane geometry and is re-traced when it lands |
