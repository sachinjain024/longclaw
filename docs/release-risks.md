...
| ~~Registry corruption strands every known project path~~ **Retired 2026-08-02 (V0-31)** | Persistence | V0-31, [plan 38](plans/completed/38-complete-step-14-recovery.md) | Met. `RegistryStore` writes `project-registry.backup.json` from the current live registry before each later save, fails closed on invalid registry JSON, has restore-from-backup and lag-by-one regression tests, and documents recovery in `apps/desktop/README.md` |
...
| ~~An unsigned build's Gatekeeper prompt reads as "this app is unsafe" to a user deciding whether to trust it with their repository~~ **Retired 2026-09-11 (LC-47)** | Release | LC-47, [the signed first-launch record](acceptance/signed-notarized-2026-09-11.md) | Met. Signed as `Developer ID Application: Sachin Jain (97864HG7U4)` and notarized; both the `.app` and the DMG are stapled, and Gatekeeper accepts both **with Apple unreachable** — `source=Notarized Developer ID` — so first launch is one click of *Open* with macOS reporting no malware found, rather than *Move to Bin* and a trip through System Settings. `npm run release:binary-audit` fails on the retired state and `npm run release:gatekeeper-check` refuses to record a phase the machine contradicts |
...
| ~~Accessibility and reduced-motion work is postponed to Step 16 and then compressed~~ **Narrowed 2026-08-04 to the screen-reader half only** | Design | **Half retired, half dated.** Keyboard access, focus order, visible focus, reduced motion and zoom were audited in Step 17 against the criteria fixed in Step 1, and they are now a run rather than a memory: `npm run a11y:audit` drives the real `App` in WebKit with no pointer input, and `--self-test` fails if a row survives a deliberate break. It found three release blockers, including that WebKit skips `<button>` in the tab order on a default Mac ([plan 41](plans/completed/41-accessibility-audit.md)). Contrast was already generated and gated by `npm run matrix`. **What is left is the screen-reader semantics — VoiceOver accessible names and the four trust states — and it is compressed exactly as this risk predicted: deferred past the MVP.** It carries a date and an owner rather than "later", which was the condition | **2026-09-04**, owner Design. Anything it finds that prevents keyboard completion is promoted and blocks |
