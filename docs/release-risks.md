...
|---|---|---|---|
| **The mid-v0 pilot never runs, and the MVP ships on internal preference.** Step 9 requires at least five completed real-repository sessions; none have run. The plan makes this a gate precisely because the alternative is guessing | Product | **Open, and being mitigated the first way: on 2026-07-31 the founder decided to run the pilot rather than proceed without it** ([decision](pilot/response-memo.md#direction-decision-2026-07-31)). Not accepted, so Waves 1–3 keep their re-rank obligation. Recruiting is the open action | M4. [The backlog](backlog/v0-backlog.md) states which of its claims this weakens, and [the memo](pilot/response-memo.md) states what closing M4 requires |
| Pilot evidence arrives after breadth is already built, so it can only be absorbed as rework | Product | ~~Wave 0 proceeds now~~ — Wave 0 closed 2026-07-31, so no breadth has been built ahead of the evidence and this risk is currently at its lowest. Waves 1–3 wait for evidence. The waves exist so that a late finding re-ranks rather than rewrites | M4 |
