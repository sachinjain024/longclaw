...
running, external file edit (the acknowledgement).
...
toasts, the acknowledgement and the archive flow all exist and broadly read correctly.
...
| D-09 | P2 | `New ticket` carries a `C` kbd chip; filter field carries a `⌘F` chip | Neither chip is rendered (no `<kbd>` outside `CommandPalette.tsx:465,491`) | Add `<kbd>` chips to the New-ticket button and the filter field. The keybindings already work. |
...
| ~~D-23~~ | P3 | Priority `None` renders as a dash glyph in the ID row | Same, but the dash sits in the chip slot with no chip — reads as a stray hyphen (see LC-108) | **Fixed 2026-08-07 (LC-85), and reopened the other way 2026-09-15 (LC-243d).** LC-85 took the first option: the dash kept its geometry and gained the frame `P1`…`P4` wear, so the five levels shared one slot, and omitting it would have made None the one level that says nothing. The frame came off again after a look at a real board. Two things the original reading missed. The frame is not the slot — it is the chip, and a chip is a thing a ticket *has*; drawn around the absence of a level it is a box around nothing, and on a young board where almost nothing is prioritised that is an empty rectangle on nearly every card, which is louder than the hyphen it was meant to fix. And the dash was never saying nothing: it is the one mark in the set that means *no level*, and it says that on its own, which is exactly how the prototype has drawn it all along (`prototype.js:102`). This was the closer call of the two, and it is the prototype that settles it — this is the ticket for doing what the prototype does. One thing was gained by the round trip: the dash is back on the 14×14 grid its master is drawn on, so `glyph-drift-guard` can compare the two for the first time, and `priority-none` moved out of that guard's `NOT_COPIED` list. |
...
priority glyph · title · acknowledgement dot · fraction · ≤2 labels · relative updated
...
| ~~D-3I~~ | P3 | Checked checklist items are struck through | Not struck through | **Answered and fixed 2026-08-07 (LC-110): keep.** `components.md:218` specifies `ink-3` + line-through for the settled checked row, and `:219` has the acknowledged agent row keep full `ink` and no strike until the ticket settles — so the strike is not a stylistic flourish to drop, it is what makes the acknowledgement legible as a distinct state. Both are implemented as `.checked` and `.acknowledged.checked`. |
...
| ~~D-4D~~ | P3 | No checklist counter in create mode | `0/0` | **Fixed 2026-08-08 (LC-119).** The counter is gone from create mode at every length. The row's two cells disagreed — `createPanelHTML` (`prototype.js:889`) draws none at all, while the plan cell asked only to hide it until a first item — and the **prototype cell wins**, as it did for D-49. A second argument was offered alongside it: every draft item was open by construction (`NewTicket.checklist` was a list of strings), so the numerator could never move. **That argument is retired (LC-242h, 2026-08-31)** — a draft row now ticks, so the numerator does move. The row stands on the prototype cell alone, which is the cell that decided it. The panel's own count keeps its place. |
...
## 17 · External update / agent acknowledgement

**Spec:** `states.md:126-152`. Card: `accent-agent-acknowledged-border`, 3px ring, **8px
...
switch to `accent-agent` while acknowledged. Decays on open, or 2 minutes after the last
write.
...
| ~~D-62~~ | P3 | Agent green is for agent writes; an unknown actor gets the **warn** treatment (`states.md:150-152`) | An unknown-actor change gets the full agent-green treatment *and* a warn triangle — the two vocabularies are mixed on one line | **Fixed 2026-08-08 (LC-148).** `acknowledgementClass` is the one place the accent is picked, and the card and the list row both ask it; it replaces `wearsAgentAccent`, which returned true for anything that was not a person. Three surfaces were wrong the same way. Every human rule was written `.ticket-row.human-fresh`, so a person's edit showed a *green* dot on a list row. The pulse halo is a hue inside a `@keyframes`, so it beat agent-green under every actor — `lc-pulse`, `lc-pulse-human` and `lc-pulse-warn` are generated from one shape now (`accent-agent-acknowledged-*`). And the **panel** was the finding a second time, caught in review: its checklist rows, fraction, meter and `❯ just now` note come from `externallyCheckedIds`, a before/after diff of the checklist that never sees an actor, so an unclaimed write filled them agent green directly under the panel's own warn banner. They read the mark now and settle with it. Colour is the whole finding and jsdom cannot see it, so `matrix` renders a second acknowledged card with no attribution and probes that its footer is `--lc-warn` and ΔE ≥ 10 from the agent's; both go red on this exact defect in all 8 axes. Closed alongside LC-183, which took the whole vocabulary from *Fresh* to *Acknowledgement* — `CONTEXT.md:33` names `Fresh` as a word to avoid — so the module, the classes and the tokens above are the renamed ones. |
...
14. ~~**D-21**~~ — column-header `+` (done 2026-08-07, LC-83, with ~~**D-22**~~
    and ~~**D-23**~~: the board's focus ring and the `None` dash, which LC-85
    framed as a chip and LC-243d unframed again — see the row itself).
...
16. ~~**D-60**~~ / ~~**D-61**~~ / ~~**D-62**~~ — acknowledgement attribution, closed
...
- ~~**D-3I**~~ — **answered 2026-08-07: keep** (LC-110). It was never only a
  ...
  it back off the acknowledged agent one, so dropping it would have cost the panel the
