...
|---|---|---|---|---|---|---|
| V0-01 | Close the atomic-replace race: hash the displaced bytes, preserve them, emit a typed conflict; evaluate `renamex_np(RENAME_SWAP)` with a no-silent-loss fallback | 14 | **Release-blocking.** An external write landing between expected-hash validation and the rename is silently overwritten *and* its watcher notification is consumed by the self-write receipt. Work disappears with no trace and no conflict. The register notes a second hash check alone is insufficient. | A deterministic barrier-based race test: an external write interleaved at the validation/replace boundary is never lost, and surfaces as a conflict rather than a success | Storage | fixed |
| V0-02 | Detect a project-event sequence gap and recover by snapshot | 14 | **Release-blocking.** The frontend drops older events but accepts a later sequence without checking for a gap, so one dropped event leaves the board silently stale while still looking live. Silent staleness is worse than a visible error in a product whose claim is that external changes appear. | Loss and reordering tests: `sequence > lastSequence + 1` suspends incremental application, requests one full snapshot, and resumes from its generation boundary | Frontend | fixed |
| V0-03 | Validate the project prefix during rebuild and ingest; degrade a mismatch rather than indexing it | 14 | A ticket directory can still carry a prefix that is not this project's key. Step 10 made the two key validators agree on the *grammar*; agreement on the *project's own* key is still unenforced, so a foreign or stale key can appear as a ticket of this project. | Rename and project-prefix coverage: a mismatched prefix degrades with a diagnostic, is never silently indexed, and the file is not rewritten | Format | fixed |
| V0-04 | Watcher recovery: `NSWorkspaceDidWakeNotification` behind `platform/macos`, overflow diagnostics, coalescing with focus recovery, and an explicit watcher-unavailable state | 14 | FSEvents drops history over sleep, wake, overflow, and removed roots, and macOS gives no `Resumed` callback while the window stays focused. A closed lid is an ordinary event on a laptop; today it can leave the app confidently wrong. | A real sleep/wake soak with the window focused, and an overflow injection: both reconcile to disk state, and an unavailable watcher says so rather than showing a live-looking board | Platform | fixed |
...
| V0-06 | Virtualize board and list lanes, subscribe through selectors, and enforce an input-to-paint budget | 16 | The spike proved the data flow, not 5,000 rendered cards. The register asks for this *before* Phase 1 breadth, because the list surface in Wave 1 is the thing that renders them. | A 5,000-ticket browser trace inside the Step 4 interaction budget, taken before the list surface ships | Frontend | fixed |
