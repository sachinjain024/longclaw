...
| P9 | **The repository cannot file its own tickets.** [The CLI caveat](v0-backlog.md#the-cli-caveat-recorded-rather-than-resolved) | Real evidence for a creation surface, but evidence about *our* workflow rather than a user's, which is why it stayed deferred. It stays deferred here too — listed so that the next person who writes a defect into `docs/plans/` knows why | Product |
...
| P11 | CLI or JSON projection of the ticket store | Useful after MVP, with the caveat above | Step 3 left it optional and it did not delay the round trip. It is what would let LongClaw track LongClaw, so it and P9 move together |
