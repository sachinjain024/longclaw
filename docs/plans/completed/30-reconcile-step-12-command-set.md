...
The stale D14 entry in `docs/design/foundations/decisions.md` was also reconciled
with the accepted P1 command set. `rg` verification confirms the remaining
`assign` references are ADR 0001 history/call-outs, not live local-mode commands;
the remaining terminal references consistently describe a visible disabled
Phase 2 placeholder.
