...
| Selected | Content (cards, rows, palette) takes `--lc-accent-human-soft`; a nav row takes `line-soft` + `ink` + 600; a **segment in a dialog** takes the `--lc-accent-human` fill and `--lc-on-accent-human`, because its neighbour is 1px away and a step of background is not a difference read at a glance; the **header's view segment** takes `--lc-accent-human-text` + 600 on the segment's own surface instead, because the one row that holds it already spends its accent fill on `New ticket` (LC-243d) — none relies on color alone | `--lc-motion-state` (120ms) |
