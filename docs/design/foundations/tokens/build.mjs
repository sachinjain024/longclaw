...
const staticLines = [];
for (const [k, v] of Object.entries(t.font)) staticLines.push(line(`font-${k}`, v));
...
for (const [k, v] of Object.entries(t.space)) staticLines.push(line(`space-${k}`, px(v)));
for (const [k, v] of Object.entries(t.size)) staticLines.push(line(`size-${k}`, px(v)));
for (const [k, v] of Object.entries(t.radius)) staticLines.push(line(`radius-${k}`, px(v)));
for (const [k, v] of Object.entries(t.border)) staticLines.push(line(`border-${k}`, px(v)));
...
const appearanceLines = (app) => {
  ...
  for (const [k, v] of Object.entries(t.color.neutral)) out.push(line(k, v[app]));
  for (const [k, v] of Object.entries(t.color.status)) {
    if (k === "note" || k === "done") continue; /* done derives from the human accent */
    out.push(line(`status-${k}`, v[app]));
  }
  for (const [k, v] of Object.entries(t.color.priority)) {
    if (k === "note") continue;
    out.push(line(`priority-${k}`, v[app]));
    if (v[`mark-${app}`]) out.push(line(`priority-${k}-mark`, v[`mark-${app}`]));
  }
  ...
  for (const [k, v] of Object.entries(t.elevation[app])) out.push(line(`shadow-${k}`, v));
  for (const [k, v] of Object.entries(t.mix[app])) out.push(line(`mix-${k}`, v));
  return out;
};
...
for (const actor of ["human", "agent"]) {
  ...
  derived.push(line(`accent-${actor}-soft`, `color-mix(in oklab, ${a} var(${P}mix-soft), var(${P}surface))`));
  derived.push(line(`accent-${actor}-wash`, `color-mix(in oklab, ${a} var(${P}mix-wash), var(${P}surface))`));
  derived.push(line(`accent-${actor}-ring`, `color-mix(in oklab, ${a} var(${P}mix-ring), transparent)`));
  derived.push(line(`accent-${actor}-hover`, `color-mix(in oklab, ${a} var(${P}mix-hover), var(${P}ink))`));
  derived.push(line(`accent-${actor}-active`, `color-mix(in oklab, ${a} var(${P}mix-active), var(${P}ink))`));
  derived.push(line(`accent-${actor}-border`, `color-mix(in oklab, ${a} var(${P}mix-border), var(${P}line))`));
...
const out = [];
out.push(`/* LongClaw design tokens v${t.$meta.version} — GENERATED by tokens/build.mjs, do not edit.`);
out.push(` * Source: tokens/design-tokens.json · Verified: scripts/a11y-check.mjs`);
out.push(` * Contract: set data-appearance="light|dark" AND data-theme="${themes.join("|")}"`);
out.push(` * on the same root element (<html>). Components consume only --lc-* tokens;`);
...
out.push("}");
out.push("");
out.push("/* Fallbacks so a bare document renders as light Indigo (the defaults). */");
out.push(":root {");
...
out.push("/* Derived accent variants — resolve against whichever theme/appearance is active. */");
out.push(":root, [data-appearance] {");
out.push(...derived);
out.push("}");
out.push("");
out.push("/* The agent pulse — the designed acknowledgement of an external file edit.");
out.push(`   Play ${t.motion["pulse-iterations"]} iterations of ${t.motion["pulse-duration"]}; never loop forever. */`);
...
out.push(`  :root { ${P}motion-fast: 0ms; ${P}motion-state: 0ms; ${P}motion-panel: 0ms; }`);
out.push("}");
out.push("");

writeFileSync(join(here, "design-tokens.css"), out.join("\n"));
console.log(`wrote design-tokens.css (${out.length} lines, ${themes.length} themes × 2 appearances)`);
