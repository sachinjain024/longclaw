...
**A third has expired, on 2026-09-20: the app is no longer network-free.** LC-256a shipped one optional, identifier-free check for a newer LongClaw ([ADR 0014](../adr/0014-one-optional-check-for-a-newer-longclaw.md)), so *no network connection* stopped being true and *no information* became the honest line — the boundary moved from what the app can connect to, to what it can disclose, and the second is the one that was ever worth promising. The pillar in §2 and the home section in §4.1 above still carry the old sentence and are left as they were written, the way the two constraints above are: this brief is the record of what was decided when, not a live copy of the site. The announcement post was revised with 0.2.0 to say what the one request is and where its off switch lives. **The home page's `Fully offline` badge stays** — it was changed to `Works offline` and changed back the same day, because the badge is a claim about *features* and no feature moved: with no connection at all every one of them behaves the same, and the only thing that does not happen is the check. It comes off when a feature needs a network, and not before. What must not come back is a claim about *traffic* — that the app makes no request at all.
