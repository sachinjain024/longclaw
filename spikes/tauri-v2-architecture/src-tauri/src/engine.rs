...
use crate::core::{
    AppError, AppResult, ErrorCode, ProjectEvent, ProjectReference, ProjectSnapshot, SearchResult,
    StreamEnvelope, TicketIndex, WriteResult,
};
...
impl ProjectEngine {
    ...
    pub fn rebuild(&self, reason: &str, emit: bool) -> AppResult<ProjectSnapshot> {
        if !self.root.is_dir() {
...
mod tests {
    ...
    use crate::core::{ErrorCode, ProjectEvent, StreamEnvelope, TicketIndex};
    ...
    fn filesystem_round_trip_covers_self_write_external_burst_deletion_and_reconcile() {
        ...
        assert!(matches!(
            resume_event.event,
            ProjectEvent::IndexRebuilt { reason, .. } if reason == "resume"
        ));

        let moved = temp.path().join("folder-moved");
        fs::rename(&root, &moved).unwrap();
        let error = engine.rebuild("resume", true).unwrap_err();
