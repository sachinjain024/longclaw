...
use core::{
    AppResult, ProjectReference, ProjectSnapshot, SearchResult, StreamEnvelope, StreamFrame,
    VisibleUiProbe, WriteResult, WriteTicketTitleRequest,
};
...
fn report_visible_ui(probe: VisibleUiProbe, app: AppHandle) -> AppResult<()> {
    ...
    println!("VISIBLE_UI_PROBE {json}");
    if let Some(started) = PROCESS_STARTED.get() {
        println!(
            "STARTUP_TO_RENDERED_MS {:.2}",
            started.elapsed().as_secs_f64() * 1_000.0
        );
    }
    if std::env::var_os("LONGCLAW_SPIKE_EXIT_AFTER_FIRST_PROBE").is_some() {
        app.exit(0);
    }
    Ok(())
}
...
pub fn run() {
    ...
    let app = tauri::Builder::default()
        ...
        .setup(|app| {
            ...
            if let Ok(root) = std::env::var("LONGCLAW_SPIKE_PROJECT") {
